...
(function() {
  ...
  Grid.prototype = {
    get: function(pos) {
      return this._values[[pos.r, pos.c]];
    },

    set: function(pos, newVal) {
      var key = [[pos.r, pos.c]];
      var oldVal = this._values[key];
      this._groups[oldVal] -= 1;

      this._values[key] = newVal;
      var g = this._groups[newVal];
      this._groups[newVal] = (g||0) + 1;
    },

    del: function(pos) {
      var key = [[pos.r, pos.c]];
      var oldVal = this._values[key];
      delete this._values[[pos.r, pos.c]];

      var g = this._groups[oldVal];
      if (g === 1) {
        delete this._groups[oldVal];
      } else {
        this._groups[oldVal] = g - 1;
      }
    },

    count: function(value) {
      return this._groups[value] || 0;
    },
