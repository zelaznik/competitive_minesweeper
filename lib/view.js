...
(function() {
  ...
  View.prototype = ({
      ...
      on('mousedown', function(e) {
        ...
        if (now.left && now.right) {
          $('canvas').bind('mousemove',
            view.highlightCellAndNeighbors.bind(view)
          );
        } else if (now.left) {
          $('canvas').bind('mousemove',
            view.highlightActive.bind(view)
          );
        }
    ...
    highlightActive: function(e) {
      this._prev = this._prev || {x: null, y: null};
      var now = this.tiles.calculateCell(e);
      if ((this._prev.r !== now.r) || (this._prev.c !== now.c)) {
        this.game.validate(now);
        this.tiles.drawSingle(now, this.ctx, {active: true});
        this.tiles.drawSingle(this._prev, this.ctx);
        this._prev = now;
      }
    },
    ...
    highlightCellAndNeighbors: function(e) {
      ...
      this.game.neighbors(pos).forEach(function(coord) {
