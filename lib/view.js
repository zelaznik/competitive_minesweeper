  ...
  View.prototype = ({
    ...
    addListeners: function(options) {
      ...
      $('canvas').bind('mousedown', function(e) {
        ...
        if (!prev.left && !now.left) {
            return;  // This is a right click.
        }
        ...
        $('canvas').bind('mouseup',function(eUp) {
          view.deactivate(eUp);
          view.draw();
    ...
    highlightActive: function(e, options) {
      ...
      if (includeNeighbors) {
        this.game.neighbors(pos).forEach(function(coord) {
          ...
          delete this._prev[[coord.r, coord.c]];
        }.bind(this));
      }

      for (var i in this._prev) {
        var coord = {r: +i[0], c: +i[1]};
      ...
      if (includeNeighbors) {
        this.game.neighbors(pos).forEach(function(coord) {
          this._prev[[coord.r, coord.c]] = true;
        }.bind(this));
      }
    ...
    deactivate: function() {
      ...
      for (var key in this._prev) {
        var coord = key.split(',');
        var pos = {r: +coord[0], c: +coord[1]};
        this.tiles.drawSingle(pos, this.ctx);
        delete this._prev[key];
      }
      delete this._prev;
    },
